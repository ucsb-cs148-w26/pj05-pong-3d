export class AI {
    constructor(paddle, ball, paddles, speed = 15, axes = ['x', 'y']) {
        ...
        this.initialRotation = { x: paddle.mesh.rotation.x, y: paddle.mesh.rotation.y, z: paddle.mesh.rotation.z };
        this.currentLerp = 0;
        this.volleyCount = 0;
    ...
    update(delta) {
        const dist = Math.abs(this.paddle.mesh.position.z - this.ball.mesh.position.z);
        ...
        this.axes.forEach((axis) => {
            ...
            const target = isApproaching ? this.ball.mesh.position[axis] + this.positionError[axis] : 0;
        ...
        if (this.doRotation && this.targetPaddle) {
            ...
            this.paddle.mesh.rotation.x = this.initialRotation.x + (targetX - this.initialRotation.x) * this.currentLerp;
            this.paddle.mesh.rotation.y = this.initialRotation.y + (targetY - this.initialRotation.y) * this.currentLerp;
        }
    }
    ...
    pickTarget() {
        ...
        if (enemyPaddles.length > 0) {
            this.targetPaddle = enemyPaddles[Math.floor(Math.random() * enemyPaddles.length)];
