...
const NOVA_CONFIG = {
    ...
    assets: [
        ...
        { key: 'particlePoints', type: 'particles', material: 'particleMaterial', count: 280 },
        {
            key: 'sparkPoints',
            type: 'particles',
...
const PIXEL_BURST_CONFIG = {
    ...
    assets: [
        { key: 'coreMesh', type: 'core', material: 'material' },
        { key: 'particlePoints', type: 'particles', material: 'particleMaterial', count: 220 },
        {
            key: 'sparkPoints',
            type: 'particles',
            material: 'particleMaterial',
...
const VORTEX_CONFIG = {
    ...
    assets: [
        ...
        { key: 'particlePoints', type: 'particles', material: 'particleMaterial', count: 360 },
        {
            key: 'sparkPoints',
            type: 'particles',
            material: 'particleMaterial',
    ...
    configureVisibility(animation) {
        ...
        if (animation.ringMesh2?.material) animation.ringMesh2.material.opacity = 1.0;
        if (animation.ringMesh3?.material) animation.ringMesh3.material.opacity = 1.0;
    },
    initParticles(animation) {
    ...
    onUpdate(animation, dt, progress) {
        ...
        if (animation.particlePoints) animation.particlePoints.rotation.y += dt * 3.4;
        ...
        for (let i = 0; i < animation.particleCount; i++) {
            ...
            const angle = data.angle[i] + animation.particleTime * spinSpeed + t * 10.0;
            animation.particlePositions[idx] = Math.cos(angle) * radius;
...
const BOOM_HEADSHOT_CONFIG = {
    ...
    assets: [
        { key: 'particlePoints', type: 'particles', material: 'particleMaterial', count: 160 },
        {
            key: 'sparkPoints',
            type: 'particles',
            material: 'particleMaterial',
...
const BLACK_HOLE_CONFIG = {
    ...
    setup(animation, materials) {
        ...
        animation.blackHoleMesh = new THREE.Mesh(blackHoleGeometry, materials.material);
        ...
        animation.gammaBurstMesh = new THREE.Mesh(gammaBurstGeometry, animation.gammaBurstMaterial);
        ...
        animation.registerColorUniformGroups([animation.gammaBurstMaterial.uniforms]);
    },
    configureVisibility(animation) {
        if (animation.visual?.scale) animation.visual.scale.setScalar(BLACK_HOLE_SCALE);
        animation.ringMesh.visible = true;
        animation.ringMesh2.visible = true;
    ...
    onUpdate(animation, dt, progress) {
        ...
        animation.blackHoleMesh.scale.setScalar((1.0 + burst * 0.3 + osc) * visibleScale);
        animation.blackHoleMesh.rotation.x += dt * 0.6;
        if (animation.ringMesh) animation.ringMesh.scale.setScalar(visibleScale);
        if (animation.ringMesh2) animation.ringMesh2.scale.setScalar(visibleScale * 1.05);
        if (animation.lensMesh) animation.lensMesh.scale.setScalar(visibleScale);
        if (animation.particlePoints) animation.particlePoints.scale.setScalar(visibleScale);
        ...
        if (animation.flareSprite) {
            animation.flareSprite.material.opacity = Math.min(1.0, 0.25 + burst * 0.9);
        ...
        for (let i = 0; i < animation.particleCount; i++) {
            ...
            const theta = -(data.angle[i] + time * (0.8 + data.speed[i] * 1.4) + eased * 7.5);
            const timePull = 0.18 + time * 0.08;
            const spiralRadius = data.radius[i] * Math.exp(-eased * (0.9 + timePull) * 0.22);
            const attraction = 0.35 / (spiralRadius * spiralRadius + 0.25);
            const pulledRadius = Math.max(0.03, spiralRadius - attraction * (0.35 + eased));
    ...
    resetCommonState(animation) {
        ...
        if (animation.particleUniforms?.uTime) animation.particleUniforms.uTime.value = 0.0;
        if (animation.lensMaterial?.uniforms?.uTime) animation.lensMaterial.uniforms.uTime.value = 0.0;
        if (animation.gammaBurstMaterial?.uniforms?.uTime) animation.gammaBurstMaterial.uniforms.uTime.value = 0.0;
        if (animation.blackHoleMesh?.rotation) animation.blackHoleMesh.rotation.set(0, 0, 0);
        if (animation.lensMesh?.rotation) animation.lensMesh.rotation.set(0, 0, 0);
        if (animation.gammaBurstMesh?.rotation) animation.gammaBurstMesh.rotation.set(0, 0, 0);
        if (animation.visual?.scale) animation.visual.scale.setScalar(BLACK_HOLE_SCALE);
        if (animation.blackHoleMesh) animation.blackHoleMesh.scale.setScalar(0);
        if (animation.ringMesh) animation.ringMesh.scale.setScalar(0);
        if (animation.ringMesh2) animation.ringMesh2.scale.setScalar(0);
...
const MONSOON_CONFIG = {
    ...
    shaders: {
        ...
        particles: {
            ...
            injections: {
                CUSTOM_POINT_SIZE_LOGIC: 'aSize * (1.0 - uProgress) * 6.0',
                CUSTOM_ALPHA_LOGIC: 'smoothstep(0.5, 0.1, d)',
                CUSTOM_COLOR_LOGIC: 'mix(uColor * 0.55, mix(uColor, vec3(1.0), 0.35), 1.0 - d)'
            }
        }
    },
    assets: [],
    setup(animation, materials) {
        ...
        animation.cloudMaterials = animation.cloudMeshes.map((mesh) => mesh.material);
        ...
        animation.rainPoints = new THREE.Points(animation.rainGeometry, materials.particleMaterial);
    ...
    onUpdate(animation, dt, progress) {
        ...
        if (animation.cloudMeshes && animation.cloudMaterials && animation.cloudOffsets) {
            for (let i = 0; i < animation.cloudMeshes.length; i++) {
                ...
                mesh.position.y = baseY + Math.sin(animation.cloudTime * 1.2 + i) * 0.24;
                mesh.position.z = base.z + Math.sin(animation.cloudTime * 1.2 + i * 0.7) * 0.12;
            }
        }
        for (let i = 0; i < animation.rainCount; i++) {
            const idx = i * 3;
            ...
            animation.rainPositions[idx + 1] += animation.rainVelocities[idx + 1] * dt;
            animation.rainPositions[idx + 2] += animation.rainVelocities[idx + 2] * dt;
...
const ORBITAL_STRIKE_CONFIG = {
    ...
    configureVisibility(animation) {
        if (animation.beamMesh) {
            ...
            if (animation.beamMesh.material) animation.beamMesh.material.opacity = 0.0;
        }
        if (animation.impactRing) {
            animation.impactRing.visible = true;
            animation.impactRing.scale.set(0.2, 0.2, 0.2);
            if (animation.impactRing.material) animation.impactRing.material.opacity = 1.0;
        }
        if (animation.scorchMark) {
            const material = animation.scorchMark.userData.scorchMaterial ?? animation.scorchMark.material;
    ...
    onUpdate(animation, _dt, progress) {
        ...
        if (animation.beamMesh) {
            ...
            if (animation.beamMesh.material) {
                animation.beamMesh.material.opacity = Math.min(1.0, width * 2.4) * globalAlpha;
            }
...
const BASS_DROP_CONFIG = {
    ...
    onUpdate(animation, dt, progress) {
        ...
        if (animation.equalizerSphere) {
            ...
            animation.equalizerSphere.scale.set(sphereScale, sphereScale, sphereScale);
    ...
    onUpdate(animation, dt, progress) {
        ...
        if (animation.gasVelocities) {
            ...
            for (let i = 0; i < animation.gasCount; i++) {
                ...
                animation.gasVelocities[idx + 2] += (Math.random() - 0.5) * dt * turbulence;
            }
        }
...
const CRYSTAL_SPIRE_CONFIG = {
    ...
    setup(animation, materials) {
        if (!animation.baseMesh) {
            ...
            if (baseMaterial.uniforms?.uColor?.value?.copy && baseMaterial.uniforms?.uColor?.value?.clone) {
            ...
            animation.baseMesh = GoalAssetFactory.createCrystalSpireBase(baseMaterial);
        ...
        for (let i = 0; i < spireCount; i++) {
            const angle = (i / spireCount) * Math.PI * 2 + (Math.random() - 0.5) * 0.35;
            const distance = 0.08 + Math.random() * 0.42;
            const height = 1.9 + Math.random() * 2.1;
            const radiusBottom = 0.2 + Math.random() * 0.28;
            const geometry = new THREE.CylinderGeometry(0.0, radiusBottom, height, 6, 1, false);
    ...
    configureVisibility(animation) {
        if (animation.baseMesh) {
            ...
            if (animation.baseMesh.material) animation.baseMesh.material.opacity = 0.88;
        }
    ...
    onUpdate(animation, dt, progress) {
        ...
        if (animation.baseMesh) {
            ...
            animation.baseMesh.scale.set(2.7 * basePulse, (0.42 + baseRise * 0.04) * basePulse, 2.7 * basePulse);
        ...
        for (let i = 0; i < animation.spireMeshes.length; i++) {
            ...
            mesh.position.y = mesh.userData.baseY - (1.0 - rise) * mesh.userData.height * 0.45;
            mesh.rotation.y += dt * (0.24 + i * 0.08);
            if (mesh.material) mesh.material.opacity = (0.26 + Math.min(1.0, rise) * 0.7) * fade;
        }
    },
    resetCommonState(animation) {
        if (animation.baseMesh) {
...
const GRAVITY_WELL_CONFIG = {
    ...
    onUpdate(animation, dt, progress) {
        ...
        if (animation.debrisPositions && animation.debrisVelocities) {
            ...
            if (!animation.debrisBursted && progress >= 0.2 && animation.debrisDirections) {
                for (let i = 0; i < animation.debrisCount; i++) {
                    const idx = i * 3;
                    const burstSpeed = animation.debrisBurstScale?.[i] ?? 18.0;
                    animation.debrisVelocities[idx] = animation.debrisDirections[idx] * burstSpeed;
                    animation.debrisVelocities[idx + 1] = animation.debrisDirections[idx + 1] * burstSpeed;
                    animation.debrisVelocities[idx + 2] = animation.debrisDirections[idx + 2] * burstSpeed;
                }
...
const SPOOKY_CONFIG = {
    ...
    initParticles(animation) {
        ...
        animation.shardSystem.init((i, system) => {
            ...
            system.positions[idx] = Math.cos(angle) * radius + (Math.random() - 0.5) * 0.08;
            system.positions[idx + 1] = animation.lastTotemY + yOffset;
            system.positions[idx + 2] = Math.sin(angle) * radius + (Math.random() - 0.5) * 0.08;
    ...
    onUpdate(animation, dt, progress) {
        ...
        const totemY = -1.55 + rise * 2.55 + Math.sin(animation.spookyTime * 2.3) * 0.08;
        ...
        if (animation.smokePositions && animation.smokeVelocities && animation.smokeAngles && animation.smokeSpin) {
            for (let i = 0; i < animation.smokeCount; i++) {
                const idx = i * 3;
                animation.smokePositions[idx + 1] += totemDeltaY * 0.55;
                const angle = animation.smokeAngles[i] + dt * (0.6 + animation.smokeSpin[i] * 1.3);
                animation.smokeAngles[i] = angle;
                ...
                if (animation.smokePositions[idx + 1] > top) {
                    ...
                    animation.smokePositions[idx + 1] = totemY - 0.8 + Math.random() * 0.35;
                    animation.smokePositions[idx + 2] = Math.sin(resetAngle) * resetRadius;
        ...
        if (
            ...
            for (let i = 0; i < animation.shardCount; i++) {
                ...
                if (animation.shardSizes && animation.shardBaseSizes) {
                    const heightFade = smoothstep(totemY + 0.2, top, animation.shardPositions[idx + 1]);
...
export function resolveGoalAnimationConfig(animationKey) {
    ...
    if (typeof animationKey === 'number') {
        return getGoalAnimationConfigByStyleValue(animationKey) ?? GOAL_ANIMATION_CONFIGS[0];
    }
    ...
    if (typeof animationKey === 'function') {
        ...
        return id ? CONFIGS_BY_ID.get(id) ?? GOAL_ANIMATION_CONFIGS[0] : GOAL_ANIMATION_CONFIGS[0];
    }
