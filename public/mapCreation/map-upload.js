	...
	if (!Array.isArray(data.vertices)) {
		...
		data.vertices.forEach((v, i) => {
			if (!Array.isArray(v) || v.length !== 3) {
				errors.push(
					`vertices[${i}]: must be an array of 3 numbers, got ${JSON.stringify(v)}`
				);
			} else {
				v.forEach((val, j) => {
					if (typeof val !== 'number' || !isFinite(val)) {
						errors.push(
							`vertices[${i}][${j}]: expected a finite float, got ${val}`
						);
					}
				});
			}
	...
	if (!Array.isArray(data.edges)) {
		...
		data.edges.forEach((e, i) => {
			if (!Array.isArray(e) || e.length !== 2) {
				errors.push(
					`edges[${i}]: must be an array of 2 integers, got ${JSON.stringify(e)}`
				);
			} else {
				e.forEach((idx, j) => {
					if (!Number.isInteger(idx) || idx < 0) {
						errors.push(
							`edges[${i}][${j}]: expected a non-negative integer, got ${idx}`
						);
					} else if (idx >= vCount) {
						errors.push(
							`edges[${i}][${j}]: index ${idx} is out of bounds (only ${vCount} vertices)`
						);
					}
				});
			}
