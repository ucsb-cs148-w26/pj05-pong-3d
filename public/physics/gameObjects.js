...
export class Box {
    ...
    get l() { return this.body.col.l; }
    get w() { return this.body.col.w; }
    get h() { return this.body.col.h; }
...
export class Boxbox {
    constructor( size ) {
        ...
        this.bodies = [
            new Box( center.clone().add(0, half_size, 0), 9999999, new THREE.MeshBasicMaterial({color: 0x00ff00}), size, size, 0.1 ),
            new Box( center.clone().add(0, -half_size, 0), 9999999, new THREE.MeshBasicMaterial({color: 0x00ff00}), size, size, 0.1 ),
            new Box( center.clone().add(half_size, 0, 0), 9999999, new THREE.MeshBasicMaterial({color: 0xff0000}), size, 0.1, size ),
            new Box( center.clone().add(-half_size, 0, 0), 9999999, new THREE.MeshBasicMaterial({color: 0xff0000}), size, 0.1, size ),
            new Box( center.clone().add(0, 0, half_size), 9999999, new THREE.MeshBasicMaterial({color: 0x0000ff}), 0.1, size, size ),
            new Box( center.clone().add(0, 0, -half_size), 9999999, new THREE.MeshBasicMaterial({color: 0x0000ff}), 0.1, size, size ),
        ]
    ...
    syncVisual() {
        for (let i = 0; i < this.bodies.length; i++) this.bodies[i].visualBody.position.copy( this.bodies[i].body.x );
    }
