...
export class PhysicsEngine {
    ...
    get dim() { return 6 * this.bodies.length; }
    ...
    getState() {
        ...
        for ( let i = 0; i < this.bodies.length; i++ ) {
            ret.load( j, this.bodies[i].x ); j += 3;
            ret.load( j, this.bodies[i].v ); j += 3;
        } 
    ...
    getDerivative() {
        ...
        for ( let i = 0; i < this.bodies.length; i++ ) {
            ret.load( j, this.bodies[i].v ); j += 3;
            ret.load( j, this.bodies[i].f.clone().scale( 1 / this.bodies[i].m ) ); j += 3;
        }
        ...
        for ( let i = 0; i < this.colliders.length; i++ ) {
            for ( let j = i + 1; j < this.colliders.length; j++ ) {
                ...
                if ( result === undefined ) {
                    ...
                    if (result === undefined) throw new Error(`Some two collision objects don't support collision against each other`);
                }
